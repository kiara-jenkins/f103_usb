...
switch	( c )
	...
	#ifdef USE_I2C
		   i2c_rxbuf[0] = 0xBE; i2c_start( c - '0' );
	#endif
		break;

	#ifdef USE_I2C
	case 'y' :
		i2c_enable(); CDC_printf("i2c enabled\n");
		break;
	case 'i' :
		CDC_printf( "cnt=%u, %02x\n", i2c_rxcnt, i2c_rxbuf[0] );
		break;
	#endif
...
while (1)
	...
	// actions 1 fois par seconde
 	if	(  ( old1Hz != cnt1Hz ) )
		...
		#endif
		if	( cntblinks == 5 )
			cntblinks = 1;		// re-armer BLUE_PRESS pour MidiAllOff
	#endif
		}
	}
}
